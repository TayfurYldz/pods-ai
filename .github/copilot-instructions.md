...
- FFmpeg setup is platform-specific: use native `apt-get install -y ffmpeg` on Ubuntu, and the pinned `FedericoCarboni/setup-ffmpeg` v2 action with `github.token` on Windows.
